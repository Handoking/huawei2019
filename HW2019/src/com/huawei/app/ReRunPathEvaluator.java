 ...
 *
 */
public class ReRunPathEvaluator implements Evaluator {
    ...
    public ReRunPathEvaluator(SkyEyes ctx) {
    	this.ctx= ctx; 
    	cars = ctx.cars;
    	roads=ctx.roads;
    	crosses=ctx.crosses;
	...
	private CarPathNode getCarPathNode(int carId,int[] roadIds) {
		...
		CarPathNode head = new CarPathNode(car.getStartPoint(),roadIds[0], null);
		Road road = roads.get(roadIds[0]);
		int anCrossId = road.getAnotherCrossId(car.getStartPoint());
...
//		System.err.println(anCrossId != car.getDesCrossId());
	...
	public int onScheduling(int carId, int curCrossId) {
		...
		CarAttr cs = ctx.statues.get(carId);
		Car car = cs.car;
		// 注意已经到达目的地，返回-1
